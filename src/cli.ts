...
program
  .hook('preAction', (_, actionCommand) => {
    colourfulUnicorn.info(
      `running command: ${actionCommand
        .name()
        .toUpperCase()}, options: ${JSON.stringify(actionCommand.opts())}`,
    );
  })
  .hook('postAction', (_, actionCommand) => {
    colourfulUnicorn.info(
      `command ${actionCommand.name().toUpperCase()} executed`,
    );
  });
...
program
  ...
  .option('--t, --type <string>', 'template type (http, graphql)', 'http')
  .action(async (options: { type?: ChartType }) => {
    try {
      await json.template(options.type);
    } catch (error) {
      colourfulUnicorn.error(
        `Error while creating template: ${JSON.stringify(error)}`,
      );
    }
  });
