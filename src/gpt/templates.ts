...
const useGPT = async (
  prompt: string,
  exampleFormat: string | null = null,
  cfg: Omit<OpenAI.Chat.ChatCompletionCreateParams, 'messages'> | null = null,
  ...
  const prompts: OpenAI.Chat.ChatCompletionCreateParams['messages'] = [
    { role: 'user', content: prompt },
    {
      role: 'system',
      content:
        'return only single value suitable for JSON primitive types based on the prompt',
    },
    {
      role: 'system',
      content: "JSON result will be always at property: 'value'",
    },
  ];
  if (exampleFormat) {
    prompts.push({
      role: 'system',
      content: `apply example format for result: ${exampleFormat}`,
    });
  }
  ...
  const completion = await configuration.chat.completions.create(
    <OpenAI.Chat.ChatCompletionCreateParams>mergedCfg,
  );
