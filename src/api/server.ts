...
const router = async () => {
  ...
  const graphlCharts = Object.values(charts).filter(
    (c): c is JsonGraphQLChart => c.type === 'graphql',
  );
  ...
  expressRouter.all(GRAPHQL_ROUTE, async (req: Request, res: Response) => {
    ...
    const body = await chart.resolveGraphqlChart(
      parser.proceed,
      gqlKeys,
      graphlCharts,
    );
  ...
  for (const [_, v] of Object.entries(charts)) {
    ...
    if (v.type === 'http') {
      ...
      if (
        typeof expressRouter[method] === 'function' &&
        !reservedUrls.includes(v.url)
      ) {
        expressRouter[method](v.url, async (req: Request, res: Response) => {
          ...
          const body = await chart.serverHttpChart(
            parser.proceed,
            structuredClone(v),
          );
          res.send(body);
