...
const resolveGraphqlChart = (
  expressionParseFn: (input: string) => Promise<unknown>,
  keys: string[],
  charts: JsonGraphQLChart[],
) => {
...
const serverHttpChart = async (
  ...
  if (arrayify > 1) {
    const promises = Array.from({ length: arrayify }, () =>
      reflection.reflectAndGenerate(
        expressionParseFn,
        chart.schema,
        chart.config.mimicMode,
      ),
    );
    const results = await Promise.all(promises);
    return results;
  }
  return await reflection.reflectAndGenerate(
    expressionParseFn,
    chart.schema,
    chart.config.mimicMode,
  );
...
const serverGraphQLChart = async (
  expressionParseFn: (input: string) => Promise<unknown>,
  chart: JsonGraphQLChart,
) => {
  const arrayify = chart.config.arrayify;
  if (arrayify > 1) {
    return Array.from(
      { length: arrayify },
      async () =>
        await reflection.reflectAndGenerate(
          expressionParseFn,
          chart.schema,
          chart.config.mimicMode,
        ),
...
const getMostSimilarArrayMatch = (
  arrs: string[][],
  input: string[],
