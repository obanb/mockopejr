...
describe('plugable server tests', () => {
  it('should test plug method', async () => {
    ...
    expect(res.data).toEqual({ hello: 'hello' });
  });
  it('should test unplug method', async () => {
    ...
    console.log('ROUTES', chartServer.info());

    const failure = await httpUtils
      .get(`${testUtils.config.localhost}:${testUtils.config.chart.port}/hello`)
